...
public class TableAttributeHandler
{
    ...
    private void parse(  Class pTableObjectClass ) {
        ...
        for( int i = 0; i < tMethods.length; i++ ) {
            ...
            if (ta != null) {
                mAttributes.put( ta.column(), new TableAttributeEnity( ta.header(), ta.column(), ta.width(), ta.decimals(), ta.editable(), m));
            }
        }
    }
    ...
    class  TableAttributeEnity implements Comparable<TableAttributeEnity> {
        ...
        TableAttributeEnity( String pHeader, int pColumn, int pWidth, int pDecimals, boolean pEditable,  Method pMethod ) {
            mColumn = pColumn;
            mHeader = pHeader;
            mMethod = pMethod;
            mWidth = pWidth;
