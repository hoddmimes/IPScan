...
public class TableModel<T> extends AbstractTableModel
{
    ...
    public TableModel(Class<T> pObjectClass ) {
        ...
        mTableColumnModel = new ColumnModel( mTableAttributeHandle.getHeaders(), mTableAttributeHandle.getAttributeTypes(),
                mTableAttributeHandle.getWidths(), mTableAttributeHandle.getDecimals(), mTableAttributeHandle.getEditables());
    }
    ...
    {
        ColumnModel( String pHeaders[], Class pClasses[], int pWidths[], int pDecimals[], boolean pEditables[]  ) {
            super();
            init( pHeaders, pClasses, pWidths, pDecimals, pEditables );
        }

        private void init( String pHeaders[], Class pClasses[], int pWidths[], int pDecimals[], boolean pEditables[]) {
            for( int i = 0; i < pHeaders.length; i++) {
                super.addColumn( new ColumnData(i, pHeaders[i], pClasses[i], pWidths[i], pDecimals[i], pEditables[i]));
            }
        }
    }
     ...
     class ColumnData extends  TableColumn {
        ...
        ColumnData(int pIndex, String pHeader, Class pClass, int pWidth, int pDecimals, boolean pEditable) {
            super();
            mDecimals = pDecimals;
            mEditable = pEditable;
